...
const detectors: Detector[] = [
  ...
  { category: "organization", label: "Named institution", severity: "high", regex: /\b(at)\s+([A-Z][\w'-]*(?:\s+[A-Z][\w'-]*){0,5}\s+(?:Medical\s+Centre|Health\s+Centre|Hospital|Clinic|University|Polytechnic|College|Institute|Chambers|Engineering\s+Services|Media\s+House))/g, token: "ORGANIZATION", captureGroup: 2, keepPrefix: 1, restorable: true },
  ...
  { category: "phone", label: "Nigerian mobile number", severity: "high", regex: /(?<![\w+])0(?:70|80|81|90|91)\d(?:[\s().-]?\d){7}(?!\w)/g, token: "PHONE", restorable: true },
...
export function analyzePrompt(input: string, mode: "balanced" | "strict" = "balanced"): PrivacyAnalysis {
  ...
  return {
    ...
    risk,
    findings,
    changes: changes.length ? [...new Set(changes)] : ["No sensitive transformation was necessary."],
    aliases,
    contentPolicy,
    policyExplanation,
    combinationRisk,
