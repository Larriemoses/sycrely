...
test("semantic benchmark contains 48 distinct balanced cases", () => {
  assert.equal(SEMANTIC_BENCHMARK.length, 48);
  assert.equal(new Set(SEMANTIC_BENCHMARK.map((row) => row.prompt)).size, 48);
  assert.equal(SEMANTIC_BENCHMARK.filter((row) => row.expected === null).length, 16);
});
...
test("local semantic baseline meets its measured calibration floor", () => {
  ...
  assert.ok(recall >= 0.7, `semantic recall regressed to ${recall.toFixed(3)}`);
  assert.ok(precision >= 0.8, `semantic precision regressed to ${precision.toFixed(3)}`);
