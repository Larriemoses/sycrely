...
import { EXPERIMENTAL_MODEL_MANIFEST } from "@/lib/model-integrity";

const MODEL_ID = EXPERIMENTAL_MODEL_MANIFEST.localModelId;
...
export default function ModelLab() {
  ...
  async function loadModel(){if(cachedPipeline){setStatus("ready");return;}setStatus("loading");setError("");const started=performance.now();try{const {env,pipeline}=await import("@huggingface/transformers");env.localModelPath=EXPERIMENTAL_MODEL_MANIFEST.localBasePath;env.allowLocalModels=true;env.allowRemoteModels=false;env.useBrowserCache=true;cachedPipeline=await pipeline("token-classification",MODEL_ID,{dtype:"q8",device:"wasm"}) as unknown as typeof cachedPipeline;setLoadMs(Math.round(performance.now()-started));setStatus("ready");}catch(reason){setError(reason instanceof Error?reason.message:"The verified local model could not be loaded. Run pnpm model:fetch first.");setStatus("failed");}}
  ...
  return <main className="model-lab-page"><section className="lab-shell">
    <header className="lab-header"><div><p className="eyebrow">LOCAL MODEL LAB</p><h1>Test the privacy model safely</h1><p>Experimental development tool. Only the synthetic sentence selected below is processed.</p></div><nav><Link href="/annotation-lab">Review training data</Link><Link href="/">← Return to Sycrely</Link></nav></header>
    <div className="lab-warning"><strong>Not connected to Private Mode</strong><span>This candidate cannot approve or send a real conversation. It loads only Sycrely&apos;s same-origin, pre-verified model files; inference runs inside this browser.</span></div>
    <section className="lab-card"><div className="lab-model-row"><div><small>Local candidate</small><strong>{MODEL_ID}</strong><span>Same origin · Quantized · WebAssembly · {(EXPERIMENTAL_MODEL_MANIFEST.bytes/1_000_000).toFixed(1)} MB · revision {EXPERIMENTAL_MODEL_MANIFEST.revision.slice(0,8)}</span></div><span className={`lab-state ${status}`}>{status.replace("idle","not loaded")}</span></div><div className="lab-actions"><button className="primary" onClick={loadModel} disabled={status==="loading"||status==="running"}>{status==="loading"?"Preparing local model…":cachedPipeline?"Model ready":"Load local model"}</button>{cachedPipeline&&<button className="secondary" onClick={runCase} disabled={status==="running"}>{status==="running"?"Detecting locally…":"Run selected test"}</button>}</div>{error&&<p className="lab-error" role="alert">{error}</p>}<div className="lab-metrics"><span><b>{loadMs===null?"—":`${loadMs} ms`}</b>First load</span><span><b>{runMs===null?"—":`${runMs} ms`}</b>Latest inference</span><span><b>{entities.length}</b>Entities found</span></div></section>
