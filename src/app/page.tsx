...
export default function Home() {
  ...
  async function send(request:PendingRequest){if(sendLockRef.current)return;sendLockRef.current=true;setSending(true);setPending(null);const original=draft.trim();setDraft("");setMessages(v=>[...v,{id:crypto.randomUUID(),role:"user",text:original,protectedText:request.analysis.protectedText,capsule:request.capsule,aliases:request.analysis.aliases}]);try{const response=await fetch('/api/inference',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({taskCapsule:request.capsule})});if(!response.ok)throw new Error('Request failed');const data=await response.json() as {message?:unknown;meta?:DeliveryMeta};if(typeof data.message!=="string")throw new Error("Invalid response");const id=crypto.randomUUID();setMessages(v=>[...v,{id,role:'assistant',text:"",delivery:data.meta}]);for(const word of data.message.split(' ')){await new Promise(r=>setTimeout(r,28));setMessages(v=>v.map(m=>m.id===id?{...m,text:(m.text+" "+word).trim()}:m));}}catch{setMessages(v=>[...v,{id:crypto.randomUUID(),role:'assistant',text:'The protected AI request could not be completed. Your original message remained in the local vault.'}]);}finally{sendLockRef.current=false;setSending(false);}}
