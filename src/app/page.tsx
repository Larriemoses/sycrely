...
import { resolveTraceContext } from "@/lib/trace";
...
type Message = { id: string; role: "user" | "assistant"; text: string; protectedText?: string; capsule?: TaskCapsule; aliases?: AliasEntry[]; delivery?: DeliveryMeta; status?: "error" };
...
export default function Home() {
  ...
  const [pending,setPending]=useState<PendingRequest|null>(null); const [sending,setSending]=useState(false); const [showTrace,setShowTrace]=useState(false); const [selectedTraceId,setSelectedTraceId]=useState<string|null>(null); const [ending,setEnding]=useState(false); const [saved,setSaved]=useState(false);
  ...
  const traceContext=useMemo(()=>resolveTraceContext(messages,selectedTraceId),[messages,selectedTraceId]);
  const tracedMessage=traceContext.selected;
  const tracedAssistant=traceContext.assistant;
  ...
  async function send(request:PendingRequest){if(sendLockRef.current)return;sendLockRef.current=true;setSending(true);setPending(null);const original=draft.trim();setDraft("");setMessages(v=>[...v,{id:crypto.randomUUID(),role:"user",text:original,protectedText:request.analysis.protectedText,capsule:request.capsule,aliases:request.analysis.aliases}]);try{const response=await fetch('/api/inference',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({taskCapsule:request.capsule})});const data=await response.json() as {message?:unknown;error?:unknown;meta?:DeliveryMeta};if(!response.ok)throw new Error(typeof data.error==='string'?data.error:'Request failed');if(typeof data.message!=="string")throw new Error("Invalid response");const restoredMessage=restoreAliases(data.message,request.analysis.aliases);const id=crypto.randomUUID();setMessages(v=>[...v,{id,role:'assistant',text:"",delivery:data.meta}]);const words=restoredMessage.split(/\s+/);const chunkSize=Math.max(1,Math.ceil(words.length/40));for(let index=0;index<words.length;index+=chunkSize){const chunk=words.slice(index,index+chunkSize).join(" ");await new Promise(r=>setTimeout(r,18));setMessages(v=>v.map(m=>m.id===id?{...m,text:(m.text+" "+chunk).trim()}:m));}}catch(error){setMessages(v=>[...v,{id:crypto.randomUUID(),role:'assistant',status:'error',text:`${error instanceof Error?error.message:'The protected AI request could not be completed.'} Your original message remained in the local vault.`}]);}finally{sendLockRef.current=false;setSending(false);}}
  ...
  return <main className="app-shell"><aside className="sidebar"><div className="brand"><Shield/><span>Sycrely</span></div><button className="new-chat" onClick={()=>{setMessages([]);setSelectedTraceId(null);const id=crypto.randomUUID();localStorage.setItem(ACTIVE_SESSION,id);setSessionId(id);}}><span>＋</span> New private session</button><div className="side-section"><p>ACTIVE SESSION</p><button className="session-item"><span className="session-dot"></span><span><strong>{messages.length?"Private research":"Untitled session"}</strong><small>{saved?"Encrypted locally":"Saving locally..."}</small></span></button></div><div className="side-bottom"><button onClick={()=>{setSelectedTraceId(null);setShowTrace(true);}}>Privacy trace</button><button onClick={()=>setSettingsOpen(true)}>Settings</button><button onClick={()=>setKey(null)}>Lock vault</button></div></aside>
    ...
    <aside className={`trace-panel ${showTrace?'open':''}`} aria-label="Privacy trace"><button className="trace-close" onClick={()=>setShowTrace(false)} aria-label="Close privacy trace">×</button><div className="trace-heading"><div><p className="eyebrow">PRIVACY TRACE</p><h2>What left this device?</h2></div>{traceContext.total>0&&<span>Message {traceContext.position} of {traceContext.total}</span>}</div>{tracedMessage?<><div className="trace-safe">Protected capsule sent</div><p className="trace-copy">{tracedMessage.protectedText}</p>{tracedMessage.capsule&&<div className="trace-capsule"><b>Protection policy</b><span>{tracedMessage.capsule.privacy.policyExplanation}</span><b>Capsule purpose</b><span>{tracedMessage.capsule.requestedOutput}</span><b>Local transformations</b><span>{tracedMessage.capsule.privacy.transformationCount}</span><b>Private aliases kept locally</b><span>{tracedMessage.aliases?.length??0}</span></div>}<div className="trace-list"><span><b>Original</b> Encrypted locally</span><span><b>Destination</b> {tracedAssistant?.delivery?.mode==='live'?`OpenRouter · ${tracedAssistant.delivery.model}`:tracedAssistant?.status==='error'?'External AI attempt failed':tracedAssistant?'Mock Sycrely API':'Waiting for delivery result'}</span><span><b>Provider privacy</b> {tracedAssistant?.delivery?.mode==='live'?'ZDR required; data collection denied':tracedAssistant?.status==='error'?'Privacy requirements remained enforced':tracedAssistant?'No external provider used':'Pending'}</span><span><b>Storage</b> No Sycrely server conversation record</span></div></>:<p className="muted">Nothing has been sent in this session yet.</p>}</aside>
