...
# Message-specific traces and owned business ideas

Cross-device testing exposed two important gaps. First, every “View protected version” action opened the newest trace instead of the trace belonging to the clicked message. The UI now stores the selected message ID and resolves its own adjacent delivery result, so the protected text, model, status, and privacy receipt stay paired correctly. Second, a detailed first-person business idea could pass through when the writer did not use exact words such as “confidential.” The local policy now treats owned product, application, platform, prototype, startup, and invention descriptions as confidential assets, understands natural secrecy phrases, and replaces the full distinguishing description—not merely the first sentence—with a local alias before provider delivery.

Follow-up hardening preserves safe task labels such as market-testing methodology while hiding the unique mechanism, avoids treating the instruction “protect confidential identifiers” as proof that an ordinary research project is secret, and prevents organization aliases from absorbing the next sentence. Long provider answers now appear in roughly 40 quick chunks instead of locking the composer during an unbounded word-by-word animation.
